...
class VectorCore:
        ...
        with self.lock:
            try:
                ...
                if len(vector_array) != self.dimension:
                    raise ValueError(f"Vector dimension {len(vector_array)} doesn't match database dimension {self.dimension}")
        ...
        with self.lock:
            try:
                ...
                if len(query_array) != self.dimension:
                    raise ValueError(f"Query vector dimension {len(query_array)} doesn't match database dimension {self.dimension}")
    ...
    def query_similar_with_distances(self, query_vector: List[float], k: int = 10) -> List[Tuple[str, float]]:
