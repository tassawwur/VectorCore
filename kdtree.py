...
class KDTree:
    ...
    def insert(self, vector: np.ndarray, doc_id: str):
        ...
        if len(vector) != self.dimension:
            raise ValueError(
                f"Vector dimension {len(vector)} doesn't match tree dimension {self.dimension}"
            )
    ...
    def _insert_recursive(
        self, node: Optional[KDNode], vector: np.ndarray, doc_id: str, depth: int
    ) -> KDNode:
    ...
    def search_nearest(
        ...
        if len(query_vector) != self.dimension:
            raise ValueError(
                f"Query vector dimension {len(query_vector)} doesn't match tree dimension {self.dimension}"
            )
        ...
        result = [
            (doc_id, distance)
            for _, doc_id, distance in sorted(best_matches, key=lambda x: x[2])
        ]
        return result
