...
class VectorCoreServer:
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    self.client_count += 1
                    print(
                        f"🔗 Client connected from {address} (Total clients: {self.client_count})"
                    )
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    print(
                        f"📥 Command from {address}: {data[:100]}..."
            ...
            self.client_count -= 1
            print(
                f"👋 Client {address} disconnected (Remaining clients: {self.client_count})"
            )
    ...
    def handle_add_command(self, args: List[str]) -> str:
        ...
        try:
            ...
            if success:
                return (
                    f"✅ Added vector for doc_id '{doc_id}' (dimension: {len(vector)})"
                )
            else:
